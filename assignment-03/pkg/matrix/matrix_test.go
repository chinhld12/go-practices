...
func TestTraverseRectangles(t *testing.T) {
	board := [][]int{
		{1, 0, 1},
		{1, 1, 0},
		{0, 1, 1},
	...
	visited := CreateBoolMatrix(len(board), len(board[0]))
	TraverseRectangles(board, 0, 0, visited, &visitedPairs)
...
func TestFindMinMaxIJ(t *testing.T) {
	...
	lowestI, highestI, lowestJ, highestJ := FindMinRectPoints(visitedPairs)
